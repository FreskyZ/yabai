...
class AddLiverRoomWidget(QWidget):
    ...
    def __init__(self,application_path):
        ...
        try:
            self.vtbList = []
            vtbs = codecs.open(os.path.join(self.application_path,'assets/vtb.csv'), 'r', encoding='utf-8')
            for line in vtbs:
                line = line.strip()
                if line:
    ...
    def collectVTBList(self, vtbList):
        try:
            vtbs = codecs.open(os.path.join(self.application_path, 'assets/vtb.csv'), 'w', encoding='utf-8')
            for line in vtbList:
                vtbs.write(line)
            vtbs.close()
            self.vtbList = []
